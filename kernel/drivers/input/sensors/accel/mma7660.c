...
static int sensor_active(struct i2c_client *client, int enable, int rate)
{
	...
	result = sensor_write_reg(client, sensor->ops->ctrl_reg, 0x59);
	if (result)
...
{	
	...
	result = sensor_write_reg(client, MMA7660_REG_SR, ( (2<<5) | (1<<3) | 1) );
	if (result) {
		dev_err(&client->dev, "%s:line=%d,error\n", __func__, __LINE__);
		return result;
	}

	result = sensor_write_reg(client, MMA7660_REG_SPCNT, 0xA0 );
	if (result) {
		dev_err(&client->dev, "%s:line=%d,error\n", __func__, __LINE__);
		return result;
	}

	result = sensor_write_reg(client, MMA7660_REG_PDET, 4 );
	if (result) {
		dev_err(&client->dev, "%s:line=%d,error\n", __func__, __LINE__);
		return result;
	}

	result = sensor_write_reg(client, MMA7660_REG_PD, 15 );
	if (result) {
		dev_err(&client->dev, "%s:line=%d,error\n", __func__, __LINE__);
		return result;
	}
	...
	if (sensor->pdata->irq_enable) {
		result = sensor_write_reg(client, MMA7660_REG_INTSU, 0xe7);
		if (result) {
			dev_err(&client->dev, "%s:line=%d,error\n", __func__, __LINE__);
			return result;
		}
